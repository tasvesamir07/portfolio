...
export const uploadFileToMediaApi = async (file) => {
...
export const parseHighlightItems = (content = '') => {
    if (!content) return [];
...
export const serializeHighlightItems = (items = []) =>
    JSON.stringify(
        items
            .map((item) => {
                if (item.type === 'text') {
...
export const RichTextEditor = ({ value = '', onChange, placeholder, fallbackClassName = 'h-64', ...props }) => {
    return (
        <Suspense
            fallback={
...
export const InlineFormatEditor = ({ value, onChange, placeholder, ...props }) => {
    return (
        <Suspense
            fallback={
...
export const HighlightItemsEditor = ({ items = [], onChange }) => {
...
export const StructuredItemsEditor = ({ items = [], onChange, itemLabel = 'Entry' }) => {
    const updateItem = (id, patch) => {
        onChange(items.map((item) => (item.id === id ? { ...item, ...patch } : item)));
    };
...
export const FileUploadField = ({ value, onChange, label, required, accept = "image/*" }) => {
...
export const GalleryBulkUploadField = ({ files = [], onChange, disabled = false }) => {
