...
const AdminSkills = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (formData.id) {
                await api.put(`/skills/${formData.id}`, structuredPayload);
            } else {
                await api.post('/skills', structuredPayload);
            }
    ...
    const handleMove = async (index, direction) => {
        ...
        try {
            await api.put('/reorder/skills', { orders });
            clearTranslationCache();
            clearResponseCache();
            fetchData();
        } catch (err) {
                        ...
                        <table className="w-full text-left border-collapse bg-white">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    <th className="py-4 px-6 font-bold uppercase text-[10px] tracking-widest text-gray-500">Skill Category</th>
                                    <th className="py-4 px-6 font-bold uppercase text-[10px] tracking-widest text-gray-500">Skills / Details</th>
