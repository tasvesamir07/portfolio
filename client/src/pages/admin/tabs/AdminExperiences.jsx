...
const AdminExperiences = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (formData.id) {
                await api.put(`/experiences/${formData.id}`, structuredPayload);
            } else {
                await api.post('/experiences', structuredPayload);
            }
    ...
    const handleMove = async (index, direction) => {
        ...
        try {
            await api.put('/reorder/experiences', { orders });
            clearTranslationCache();
            clearResponseCache();
            fetchData();
        } catch (err) {
                                    ...
                                    <tr key={item.id || idx} className="border-b border-gray-100 hover:bg-gray-50 transition-colors group">
                                        <td className="py-4 px-6">
                                            <div className="font-bold text-gray-900 text-base leading-tight">
                                                {item.position || 'Experience Record'}
                                            </div>
