...
const AdminResearch = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (formData.id) {
                await api.put(`/research/${formData.id}`, structuredPayload);
            } else {
                await api.post('/research', structuredPayload);
            }
    ...
    const handleMove = async (index, direction) => {
        ...
        try {
            await api.put('/reorder/research', { orders });
            clearTranslationCache();
            clearResponseCache();
            fetchData();
        } catch (err) {
                        ...
                        <table className="w-full text-left border-collapse bg-white">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    <th className="py-4 px-6 font-bold uppercase text-[10px] tracking-widest text-gray-500">Project Title</th>
                                    ...
                                    <tr key={item.id || idx} className="border-b border-gray-100 hover:bg-gray-50 transition-colors group">
                                        <td className="py-4 px-6">
                                            ...
                                            <div className="text-[10px] font-mono text-gray-400 mt-0.5 uppercase">
                                                Status: {item.status || 'N/A'} ({item.date_text || 'No date'})
                                            </div>
                                        </td>
