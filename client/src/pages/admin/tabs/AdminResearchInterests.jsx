...
const AdminResearchInterests = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (formData.id) {
                await api.put(`/research-interests/${formData.id}`, structuredPayload);
            } else {
                await api.post('/research-interests', structuredPayload);
            }
    ...
    const handleDelete = async (id) => {
        openConfirmModal(
            ...
            async () => {
                try {
                    await api.delete(`/research-interests/${id}`);
                    clearTranslationCache();
                    clearResponseCache();
                    fetchData();
    ...
    const handleMove = async (index, direction) => {
        ...
        try {
            await api.put('/reorder/research_interests', { orders });
            clearTranslationCache();
            clearResponseCache();
            fetchData();
        } catch (err) {
                        ...
                        <table className="w-full text-left border-collapse bg-white">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    <th className="py-4 px-6 font-bold uppercase text-[10px] tracking-widest text-gray-500">Interest Area</th>
