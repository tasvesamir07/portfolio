...
const queryWithTable = async <T>(fn: () => Promise<T>): Promise<T> => {
    try {
        return await fn();
    } catch (err: any) {
...
router.get('/', async (req: Request, res: Response) => {
    try {
        ...
        logger.error({ err }, 'team-members GET error');
...
router.post('/', authenticateToken, validate(teamMemberSchema), async (req: Request, res: Response) => {
    ...
    try {
        const data = await queryWithTable(async () => {
        ...
        logger.error({ err }, 'team-members POST error');
        errorResponse(res, 500, 'An internal error occurred.', err);
    }
});
...
router.put('/:id', authenticateToken, validate(teamMemberSchema), async (req: Request, res: Response) => {
    ...
    try {
        ...
        logger.error({ err }, 'team-members PUT error');
        errorResponse(res, 500, 'An internal error occurred.', err);
    }
});
...
router.delete('/:id', authenticateToken, async (req: Request, res: Response) => {
    try {
        ...
        logger.error({ err }, 'team-members DELETE error');
        errorResponse(res, 500, 'An internal error occurred.', err);
    }
});
